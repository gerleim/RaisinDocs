...
**It could reach further than RaisinDocs, but it never did — checked 2026-09-23.** RaisinTerminal2 embeds this editor three times, in the task detail, the tasks panel and the attachment editor, and keeps a `DocsEditorState` for each, which carries `SpellCheckEnabled`. An exception out of one of them does end the terminal and every session in it: RT2's `App` has handlers, but its dispatcher one logs the exception as Critical and then sets `args.Handled = false` — an HWND focus race is the one thing it swallows — so an unexpected throw is let through to end the process on purpose, having been recorded first. What kept a held dictionary from being that throw is that spell check is never on there. Nothing in the whole RaisinTerminal repository mentions it: no menu, no `SetSpellCheckEnabled` call, and the flag defaults off, so the state RT2 persists can only ever carry off. The single path that writes a dictionary is the context menu's Add to Dictionary, which `ContextMenuHandler` builds only when spell check is enabled. So the reach was latent rather than live, and becomes real the day RT2 switches spell check on — the severity above rests on RaisinDocs.Editor, whose View menu does switch it on. RT2 takes RaisinDocs by project reference, not as a package, so the fix below is in it at its next build.
...
One gap stays open: there is no way to keep both versions at once — saving yours beside the file before loading theirs would be the shape of it, if it is ever wanted.
