...
At the next start the editor offers what it finds: Yes opens each with its unsaved changes, No discards them, Cancel asks again next time. A document goes back to its file if the file is still there — into the tab the session already opened for it, or a new one — and comes back dirty, so closing it asks as it would have before the crash. It keeps the version it was based on, so if another program changed the file since the crash, saving asks first. One whose file is gone, or that never had one, comes back untitled. `DocsCanvas.RestoreUnsavedText` is what makes it dirty: it replaces the text while the document stays compared against the file's.
