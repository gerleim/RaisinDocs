...
- Compare the two versions of an externally changed file. The editor asks which version wins and says what each answer costs, so nothing is lost silently, but Yes and No are the only answers: one side's work goes. Every editor that solves this properly offers a diff rather than a second file — VS Code's Compare against *the content of the file is newer*, Emacs's `(r)evert (d)iff (y)es`, JetBrains's merge dialog — because two versions of one file is a merge, and saving yours beside theirs only moves the problem. Needs a diff view, which the editor does not have, so it is a feature rather than the third button it looks like. What detects the change is already there: `FileChangeWatcher`, `SettledFile.Read` and the per-tab `DiskStamp`.