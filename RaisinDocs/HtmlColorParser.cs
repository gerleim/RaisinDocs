...
{
    private readonly record struct ColoredSegment(string Text, RgbColor? Foreground, RgbColor? Background);
    ...
    private static List<List<ColoredSegment>>? ParseHtmlFragment(string html)
    {
        ...
        while (pos < preEnd)
        {
            if (html[pos] == '<')
            {
                ...
                {
                    FlushText(textBuf, currentLine, null, null);

                    int tagClose = html.IndexOf('>', pos + 5);
                    if (tagClose < 0) break;

                    var (fg, bg) = ParseStyleFromTag(html.AsSpan(pos, tagClose - pos + 1));
                    if (fg != null || bg != null) hasAnyColor = true;
                    ...
                    while (spanPos < spanEnd)
                    {
                        ...
                        {
                            FlushText(spanText, currentLine, fg, bg);
